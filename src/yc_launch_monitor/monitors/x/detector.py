"""Signal detector for identifying YC and Speedrun launch/acceptance signals in X posts."""
...
YC_ACCEPTANCE_PATTERNS = [
    ...
    r"\bthrilled\s+to\s+announce.*?\byc\b",
...
# Patterns that indicate purely educational / commentary / casual mentions rather than an acceptance signal
EXCLUSION_PATTERNS = [
    r"\bapplying\s+to\s+yc\b",
    r"\bapplied\s+to\s+yc\b",
    r"\bapplication\s+(?:tips|advice|guide)\b",
...
class DetectionResult:
    """Outcome of analyzing a text for YC/Speedrun signals."""
...
class XSignalDetector:
    """Analyzes text and metadata from X posts to detect founder acceptance/launch announcements."""
    ...
    def detect(
        ...
        """Evaluate if the post contains an actionable YC or Speedrun acceptance/launch signal."""
        clean_text = text.strip()
        if not clean_text:
            return DetectionResult(is_relevant=False)
        ...
        for excl_pattern in EXCLUSION_PATTERNS:
            if re.search(excl_pattern, lower_text):
                # Verify whether there's an explicit acceptance override
                ...
                if not has_explicit_acceptance:
                    return DetectionResult(is_relevant=False, signal_reason="Matched exclusion pattern")
        ...
        # Step 4: Check explicit batch mention (e.g., "YC S26", "YC W27")
        batch = self._extract_batch(clean_text)
        if not (is_yc or is_speedrun) and batch:
            # If explicit batch format like "YC S26" is mentioned
            if "yc" in lower_text:
                is_yc = True
        ...
        company_name = self._extract_company_name(clean_text, author_name, author_username)
    ...
    def _extract_company_name(
        ...
        # Heuristic 1: Look for explicit @Handle mentions after founder verbs
        handle_patterns = [
            r"(?:building|founded|founder of|creator of|we at|team at|at|for)\s+@([A-Za-z0-9_]+)",
        ...
        for pattern in handle_patterns:
            ...
            if match:
                ...
                if handle.lower() not in ("ycombinator", "speedrun", "yc", "paulgraham", "sama", "garrytan"):
                    return handle

        # Heuristic 2: Look for capitalized names following founder phrases
        name_patterns = [
            r"(?:building|founded|creator of|we at|team at|at)\s+([A-Za-z0-9_\.]+)",
        ...
        for pattern in name_patterns:
            ...
            if match:
                ...
                if candidate.lower() not in stop_words and "-" not in candidate and len(candidate) > 1:
                    return candidate

        # Heuristic 3: Author name if it looks like a startup company account
        if author_name and any(term in author_name.lower() for term in ("ai", "labs", "hq", "app", "tech", "io")):
            return author_name.strip()

        # Heuristic 4: Author display name fallback
        return author_name or author_username or None
